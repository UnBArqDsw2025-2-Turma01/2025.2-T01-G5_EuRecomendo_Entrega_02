...
- **Fluxo de Login/Cadastro** - [Bruno Bragança](https://github.com/BrunoBReis) 
- **Fluxo de Pesquisa/Busca** - [Jose Eduardo Prado](https://github.com/jevprado) 
- **Fluxo de Avaliação de Livros** - [Euller Júlio](https://github.com/Potatoyz908)
- **Fluxo de Formulário/Perfil** - [Tiago Balieiro](https://github.com/TiagoBalieiro) 
...
O diagrama de atividades de pesquisa modela o processo completo de busca de livros realizado pelo usuário no sistema EuRecomendo. Este fluxo é essencial para a experiência do usuário, pois possibilita a descoberta de conteúdos de maneira rápida e organizada, permitindo pesquisas por diferentes critérios, como título, autor ou categoria.

#### Características do Diagrama

-   **Entrada**: O fluxo é iniciado por uma ação explícita do usuário: o acesso ao campo de pesquisa, sendo que a principal entrada de dados para o sistema é o texto que o usuário digita no campo de busca.
-   **Processo Principal**: O processo central consiste em uma busca que opera em paralelo ou de forma abrangente sobre três critérios distintos: Título, Autor e Categoria. Há uma etapa crucial de verificação ("O campo digitado é válido?"), que funciona como um portão de qualidade, garantindo que apenas entradas válidas prossigam para a fase de resultados.
-   **Saída**: O diagrama possui três saídas distintas e claramente definidas, cada uma representando um resultado final diferente para o fluxo: Saída de Sucesso, Saída de Erro e Saída de Abandono (não pesquisar).


#### Elementos Principais do Fluxo

1. **Inicialização -** Acesso ao campo de pesquisa: ponto inicial da interação.  

2. **Escolha de Filtros -** Pesquisa por título, autor ou categoria: Opções de filtragem escolhidas pelo usuário.  

3. **Validação -**  Validação do campo: Checagem de consistência da entrada do usuário.  

4. **Tratamento de Resultados -** 
   - Resultados da pesquisa: Exibição de itens encontrados, levando à tela de avaliação.  
   - Mensagem de erro: Resposta para entradas inválidas.  
   - Exibição da tela principal: Caminho alternativo quando não há entrada de pesquisa.  

5. **Finalização -** Encerramento do fluxo após exibição da tela de avaliação, mensagem de erro ou retorno à tela principal.  

#### Pontos de Decisão

1. **Usuário digita no campo de pesquisa?**  
   - **Sim:** Avança para escolha do tipo de pesquisa.  
   - **Não:** Retorna à tela principal.  

2. **O campo digitado é válido?**  
   - **Sim:** Apresenta resultados e permite acesso à tela de avaliação.  
   - **Não:** Exibe mensagem de erro e finaliza o fluxo.  

#### Fluxos Alternativos

- **Fluxo sem entrada**: O usuário não digita nada e é redirecionado para a tela principal.  
- **Fluxo de erro**: O usuário insere dados inválidos, o sistema retorna uma mensagem de erro e finaliza o processo.  
- **Fluxo principal bem-sucedido**: O usuário digita, usando um filtro válido e visualiza os resultados com possibilidade de avaliação.  


#### Aspectos Técnicos

- **Validação de entrada**: Importante para evitar consultas inválidas ou inconsistentes.  
- **Uso de filtros**: Garante que a busca seja mais precisa e segmentada.  
- **Tratamento de exceções**: Exibição de mensagens claras ao usuário em caso de erro.  
- **Otimização da busca**: Pode ser implementada com indexação de dados, cache de resultados recentes e paginação para grandes volumes.  

![Diagrama de Atividades - Pesquisa](/assets/DiagramaDeAtividadesPesquisa.png)

*Figura 4: Diagrama de Atividades do Fluxo de Pesquisa - Desenvolvido por José Eduardo Prado*
...
*Figura 5: Diagrama de Atividades do Fluxo do Formulário - Desenvolvido por Tiago Balieiro*
...
|  `1.4` | 20/09 | Adição do diagrama de avaliações (Pesquisa) | [Jose Eduardo Prado](https://github.com/jevprado) | [Euller Júlio](https://github.com/Potatoyz908) |